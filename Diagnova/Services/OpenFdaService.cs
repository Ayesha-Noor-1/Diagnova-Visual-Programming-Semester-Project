...
public sealed class OpenFdaService : IOpenFdaService
{
    ...
    public async Task<OpenFdaDrugLabel?> SearchDrugLabelAsync(string query, CancellationToken cancellationToken = default)
    {
        ...
        var search =
            $"(openfda.generic_name:\"{term}\" OR openfda.brand_name:\"{term}\")";
        ...
        try
        {
            ...
            {
                _logger.LogWarning("OpenFDA HTTP {Status}: {Body}", response.StatusCode, json);
                return null;
            }
        ...
        {
            _logger.LogError(ex, "OpenFDA search failed");
            return null;
        }
    }
